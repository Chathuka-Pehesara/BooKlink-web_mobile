...
import { countUnreadExchangeMessages, countUnreadWishlistMessages } from '../lib/chatUnreadCounts.js';
...
/**
 * Unified inbox rows for swap + wanted-book threads, including unread counts for notifications.
 */
export async function loadUnifiedInboxChats(me) {
  ...
  const exchangeUnread = await Promise.all(requestRows.map((r) => countUnreadExchangeMessages(me, r)));

  const exchangeChats = requestRows.map((r, i) => {
  ...
  const wishlistUnread = await Promise.all(threadRows.map((t) => countUnreadWishlistMessages(me, t)));

  const wishlistChats = threadRows.map((t, i) => {
    const imSeeker = t.seekerClerkUserId === me;
    const peerId = imSeeker ? t.helperClerkUserId : t.seekerClerkUserId;
    const wishlistRole = imSeeker ? 'poster' : 'helper';
    ...
    return {
      ...
      lastMessageSenderClerkUserId: last?.senderClerkUserId || null,
      unreadCount: wishlistUnread[i],
    };
  });

  const allPeerIds = [...exchangeChats.map((c) => c.peerClerkUserId), ...wishlistChats.map((c) => c.peerClerkUserId)];
